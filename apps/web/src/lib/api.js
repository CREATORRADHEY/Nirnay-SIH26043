...
export const ENABLE_DEMO_FALLBACK = process.env.NEXT_PUBLIC_ENABLE_DEMO_FALLBACK === "true";
export const DEMO_REVIEWER_ACTOR_ID = process.env.NEXT_PUBLIC_DEMO_REVIEWER_ACTOR_ID || "d99c55a9-e4d4-42c1-abd1-5e9ccb2ad67c";
...
export const DEMO_CHALLENGES = [
    ...
    },
    {
        ...
        summary: "Rapid urban expansion in Ranchi district has caused groundwater depletion and inadequate rainwater harvesting infrastructure across municipal wards.",
        description: "Semi-urban towns in Ranchi district face acute seasonal water scarcity due to unplanned ground extraction, lack of runoff rainwater harvesting in public buildings, and inefficient distribution. Municipal authorities lack real-time hydrological data and localized filtration models to manage community supply during dry months.",
        domain: "Environment",
    ...
    },
    {
        ...
        summary: "Perishable vegetable crops in East Singhbhum spoil rapidly post-harvest due to lack of affordable grid-independent cold storage solutions.",
        description: "Smallholder farmers in East Singhbhum suffer up to 35% crop loss during peak harvest season due to high ambient temperatures and unreliable grid power. Existing commercial cold storage units are centralized, expensive, and logistically unviable for micro-farmers.",
        domain: "Agriculture",
        source_type: "Government Portal",
        ...
        items: [
            {
                ...
                description: "Hydrogeological field survey of 12 wards in Ranchi showing 4.2m average drop in groundwater table over 3 years.",
    ...
    "c0010000-0000-0000-0000-000000000001": [
        {
            ...
            version: 1,
            rationale: "Problem involves complex hydrogeological technology, requiring interdisciplinary academic research partnership and pilot testing before municipal scaling.",
...
const demoHEIOrganizationsStore = [
    {
...
export async function fetchChallenges(params) {
    try {
        ...
        if (process.env.NEXT_PUBLIC_ENABLE_DEMO_FALLBACK !== "true")
            throw err;
        ...
        if (params?.domain) {
            filtered = filtered.filter((c) => c.domain.toLowerCase() === params.domain?.toLowerCase());
        }
        if (params?.source_type) {
            filtered = filtered.filter((c) => c.source_type.toLowerCase() === params.source_type?.toLowerCase());
        }
...
export async function fetchChallengeEvidence(challengeId) {
    try {
...
export async function fetchQualificationHistory(challengeId) {
    try {
...
export async function fetchLatestQualification(challengeId) {
    try {
...
export async function createQualificationDecision(challengeId, payload) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/challenges/${challengeId}/qualification-decisions`;
...
export async function fetchHEIOrganizations() {
    try {
...
export async function fetchHEICandidates(challengeId) {
    try {
...
export async function createHEICandidate(challengeId, payload) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/challenges/${challengeId}/hei-candidates`;
        const res = await fetch(url, {
            method: "POST",
...
export async function fetchOrganizationCapabilities(organizationId) {
    try {
        ...
        if (process.env.NEXT_PUBLIC_ENABLE_DEMO_FALLBACK !== "true")
            throw err;
        const foundOrg = demoHEIOrganizationsStore.find((o) => o.organization_id === organizationId);
        const caps = (foundOrg?.active_capabilities || []).map((c) => ({
            ...c,
...
const demoCommitmentsStore = {};
const demoReadinessConditionsStore = {};
const demoReadinessDecisionsStore = {};
export async function fetchCommitments(challengeId, organizationId, commitmentType) {
    try {
        ...
        if (commitmentType)
            params.append("commitment_type", commitmentType);
        if (params.toString())
            url += `?${params.toString()}`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok)
...
export async function fetchCommitmentHistory(challengeId, organizationId, commitmentType) {
    try {
        ...
        const items = all.filter((c) => c.organization_id === organizationId && c.commitment_type === commitmentType);
        return { data: { items, total: items.length }, isDemo: true };
    }
}
export async function createCommitmentVersion(challengeId, payload) {
    try {
        ...
        const all = demoCommitmentsStore[challengeId] || [];
        const series = all.filter((c) => c.organization_id === payload.organization_id && c.commitment_type === payload.commitment_type);
        ...
        if (latestDec && latestDec.status === "PILOT_READY") {
            ...
            const isAffected = conds.some((cond) => cond.commitment_dependency_ids?.some((depId) => {
...
export async function fetchReadinessConditions(challengeId) {
    try {
...
export async function fetchLatestReadinessConditions(challengeId) {
    try {
...
export async function createReadinessCondition(challengeId, payload) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/challenges/${challengeId}/readiness-conditions`;
        const res = await fetch(url, {
            method: "POST",
...
export async function fetchReadinessHistory(challengeId) {
    try {
...
export async function fetchLatestReadinessDecision(challengeId) {
    try {
...
export async function createReadinessDecision(challengeId, payload) {
...
const demoOperationalStatesStore = {
...
const demoOutcomesStore = {};
export async function createPilot(challengeId, payload) {
    try {
...
export async function fetchChallengePilots(challengeId) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/challenges/${challengeId}/pilots`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok)
...
export async function fetchPilotDetail(pilotId) {
    try {
...
export async function createPilotOperationalState(pilotId, payload) {
    try {
...
export async function fetchPilotOperationalHistory(pilotId) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/pilots/${pilotId}/operational-states`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok)
...
export async function fetchLatestPilotOperationalState(pilotId) {
    try {
...
export async function createPilotEvidencePlan(pilotId, payload) {
    try {
...
export async function fetchPilotEvidencePlanHistory(pilotId) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/pilots/${pilotId}/evidence-plans`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok)
...
export async function fetchLatestPilotEvidencePlan(pilotId) {
    try {
...
export async function createOutcomeAssessment(pilotId, payload) {
...
export async function fetchOutcomeHistory(pilotId) {
    try {
        const url = `${getApiBaseUrl()}/api/v1/pilots/${pilotId}/outcomes`;
        const res = await fetch(url, { cache: "no-store" });
        if (!res.ok)
...
export async function fetchLatestOutcomeAssessment(pilotId) {
    try {
...
export async function fetchAdminOrganizations(status, org_type, page = 1, limit = 20) {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    if (status)
        params.append("status", status);
    if (org_type)
        params.append("org_type", org_type);
...
export async function updateAdminOrganizationStatus(orgId, status, rationale) {
...
export async function fetchAdminUsers(role, is_active, search, page = 1, limit = 20) {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    if (role)
...
export async function updateAdminUserStatus(userId, is_active, rationale) {
...
export async function updateAdminUserRole(userId, new_role, rationale) {
...
export async function fetchAdminAuditLogs(event_type, actor_id, page = 1, limit = 20) {
    const params = new URLSearchParams({ page: page.toString(), limit: limit.toString() });
    if (event_type)
...
export async function fetchAdminAIOperations() {
