...
class OrgStatusUpdatePayload(BaseModel):
    status: Optional[str] = Field(None, description="Target status: ACTIVE, PENDING, REJECTED, SUSPENDED")
    target_status: Optional[str] = Field(None, description="Alternative key for target status")
...
def get_admin_overview(
    ...
    open_clarifications = db.query(func.count(ClarificationRequest.id)).filter(ClarificationRequest.status == "OPEN").scalar() or 0
    ...
    total_pilots = db.query(func.count(Pilot.id)).scalar() or 0
    ...
    return {
        ...
        "total_challenges": innovation_challenges,
        "active_pilots": total_pilots,
...
def list_organizations_admin(
    ...
    for o in items:
        org_list.append({
            ...
            "members_count": db.query(func.count(OrganizationMembership.id)).filter(OrganizationMembership.organization_id == o.id).scalar() or 0,
...
def get_organization_detail_admin(
    ...
    for m, a in memberships:
        members_list.append({
            ...
            "actor_id": str(a.id),
            "name": a.display_name,
            "email": a.account.email if a.account else None,
...
def update_organization_status_admin(
    ...
    target = (payload.status or payload.target_status or "").upper()
    ...
    event_map = {
        ...
        "PENDING": "ORG_STATUS_CHANGED",
    }
    audit_entry = SecurityAuditLog(
        event_type=event_map.get(target, "ORG_STATUS_CHANGED"),
        actor_id=str(admin.id),
        details=f"Admin {admin.display_name} transitioned organization '{org.name}' ({org.id}) status from {old_status} to {target}. Rationale: {payload.rationale}",
    )
    db.add(audit_entry)
    db.commit()
    db.refresh(org)
...
def list_users_admin(
    ...
    if search:
        query = query.filter(Actor.display_name.ilike(f"%{search}%"))
...
def update_user_status_admin(
    ...
    return {
        ...
        "email": target_actor.account.email if target_actor.account else None,
